...
class focdefres_rndjob(job.slurmjob):
  ...
  def __init__(self,pars,jobname,maxnum=9999,parpath=".",logpath=".",user='joseph29',verb=False):
    ...
    super(focdefres_rndjob,self).__init__(logpath,user,verb)
    # Create the par file for this job
    self.pfname = parpath + '/' + jobname + self.jobid + '.par'
    self.write_focdefres_rndpar(self.pfname,pars,maxnum)

  def write_focdefres_rndpar(self,name,pars,maxnum):
