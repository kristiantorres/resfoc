...
for idz in progressbar(dzs, "Example: ", 40):
  ...
  # Put in fault
  mb.fault(begx=0.5,begy=0.5,begz=0.5,daz=idz,dz=7000.0,azim=180.0,theta_die=12.0,theta_shift=4.0,dist_die=0.3,perp_die=0.5)
  ...
  axarr[0].set_title('daz=%f'%idz)
  axarr[1].imshow(lbl[:-40,:,300],cmap='jet')
  axarr[1].set_title('daz=%f'%idz)
  plt.savefig(('./fig/dazmovie/daz-%s.png'%(create_inttag(idz,maxdz))),bbox_inches='tight')
  plt.show()
