...
logpath = "./log"
wrkrs,status = launch_pbsworkers(cfile,nworkers=30,queue='default',
...
vaxes,vel = sep.read_file("sigsbee_veloverwpos.H")
...
icnkr.set_image_pars(ntx=16,nthrds=16,nrmax=20,nhx=20,sverb=True)
gen = iter(icnkr)
...
sep.write_file("sigextimgwrngpos.H",imgt,os=[oz,oxi,0.0,ohx,0.0],ds=[dz,dxi,dy,dhx,1.0])
