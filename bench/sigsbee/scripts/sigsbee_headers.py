...
fdat = segyio.open("./dat/sigsbee2a_nfs.sgy",ignore_geometry=True)
fvel = segyio.open("./dat/sigsbee2a_migvel.sgy",ignore_geometry=True)
...
osx = srcx[0]; dsx = srcx[1] - srcx[0]; nsx = len(srcx)
...
for isx in range(nsx):
  ...
  recx.append(recs)
  isht  = data[sidxs,:]
  mut   = mute(isht,dt=dt,dx=0.02286,v0=1.5,tp=0.3,half=False)
