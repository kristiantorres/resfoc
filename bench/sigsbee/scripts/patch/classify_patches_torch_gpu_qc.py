...
sig_def = SigsbeeFocData('/net/thing/scr2/joseph29/sigsbee_fltres.h5')
foc_seg = SigsbeeFltSegData('/net/thing/scr2/joseph29/sigsbee_focseg.h5')
def_seg = SigsbeeFltSegData('/net/thing/scr2/joseph29/sigsbee_resseg.h5')
...
for iex in range(nex):
  ...
  fstk = foc_seg[idx]['img'].unsqueeze(0).to(device)
  slbl = foc_seg[idx]['lbl'].numpy().squeeze()
  # Defocused
  cubd = sig_def[idx]['img'].numpy().squeeze()
  defl = sig_def[idx]['lbl'].numpy().squeeze()
  dstk = def_seg[idx]['img'].unsqueeze(0).to(device)
  slbl = def_seg[idx]['lbl'].numpy().squeeze()
  # Make a prediction on the stacks
  with torch.no_grad():
    fprd = torch.sigmoid(net(fstk)).cpu().numpy().squeeze()
    dprd = torch.sigmoid(net(dstk)).cpu().numpy().squeeze()
