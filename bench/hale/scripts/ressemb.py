...
for ix in range(0,nx,50):
  plot_anggatrhos(stormw[:,:,:,:850],ix,dz,dx,oro,dro,ox=ox,show=False,pclip=0.6,fontsize=fsize,ticksize=fsize,
                  imgaspect=2.0,roaspect=0.01,figname='./fig/ressemb/imgline%d'%(ix))
  # Plot the picked
  plot_rhopicks(stormw[:,ix,:,:850],semb[ix,:,:850],rho[ix,:850],dro,dz,oro,show=False,angaspect=0.01,
                vmin=smin,vmax=smax,wspace=0.1,rhoaspect=0.04,pclip=1.1,figname='./fig/ressemb/imgline%d'%(ix))
...
ax.imshow(stkw[40,:850].T,interpolation='bilinear',cmap='gray',vmin=kmin,vmax=kmax,
...
ax.set_zlabel('Z (km)',fontsize=fsize)
...
sep.write_file("halestk.H",stkw[40].T,os=[oz,ox],ds=[dz,dx])
