...
vaxes,ivel = sep.read_file("hale_veltr.H")
ivel = np.ascontiguousarray(ivel.reshape(vaxes.n,order='F')).astype('float32')*0.001
raxes,iref = sep.read_file("hale_reftr.H")
...
wei = geom.coordgeom(nx,dx,ny,dy,nz,dz,ox=ox,nrec=nrecw,srcxs=srcxw,recxs=recxw)
...
#sep.write_file("hale_trdata.H",dat.T,ds=[d1,1.0])

#sep.write_file("hale_trimg.H",img,os=[oz,0.0,ox],ds=[dz,dy,dx])
