...
iaxes,img = sep.read_wind("hale_deftrimgs_off.H",nw=1,fw=2)
img  = img.reshape(iaxes.n,order='F')
imgt = np.ascontiguousarray(img.T).astype('float32')
imgtw = imgt[:,0,140:652,:450]

# Get axes
[nhx,nx,nz] = imgtw.shape; [oz,ox,oy,ohx,om] = iaxes.o; [dz,dx,dy,dhx,dm] = iaxes.d
...
sep.write_file("resmigtrn.H",rangs.T,ds=[dz,da,dx,odro],os=[oz,oa,ox+20*dx,ooro])
del rangs; del rmig

rangst = off2angkzx(rmigt,ohx,dhx,dz,na=na,nthrds=20,transp=True,rverb=True)
sep.write_file("resmigtrnt.H",rangst.T,ds=[dz,da,dx,odro],os=[oz,oa,7.37,ooro])
