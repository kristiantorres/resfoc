...
allx,ally = load_alldata("/net/thing/scr2/joseph29/hale_fltseg.h5",None,1)
#allx,ally = load_alldata("/net/thing/scr2/joseph29/hale_fltseg128.h5",None,1)
xshape = allx.shape[1:]
yshape = ally.shape[1:]
...
#model = unetxwu(input_size=(128,128,1))

# Set GPUs
tf.compat.v1.GPUOptions(allow_growth=True)

# Create callbacks
checkpointer = ModelCheckpoint(filepath="/scr1/joseph29/hale_fltseg-chkpt.h5", verbose=1, save_best_only=True)
...
sep.write_file("hale_fltsegloss.H",lossvepch)
vlssvepch = np.asarray(history.history['val_loss'])
sep.write_file("hale_fltsegvlss.H",vlssvepch)

# Save the accuracy history
accvepch = np.asarray(history.history['acc'])
sep.write_file("hale_fltsegacc.H",accvepch)
vacvepch = np.asarray(history.history['val_acc'])
sep.write_file("hale_fltsegvac.H",vacvepch)
