...
cluArgs.add_argument("-tjobs",help="Total number of jobs to run [145]",type=int)
cluArgs.add_argument("-njobs",help="Number of jobs to keep active at once (one job per node) [30]",type=int)
cluArgs.add_argument("-queue",help="Queue on which to submit job [sep]",type=str)
cluArgs.add_argument("-submit",help="Run a trial (no job submissions) for testing [n]",type=str)
...
nex = args.nex # Number of examples within a job
queue = args.queue
submit = sep.yn2zoo(args.submit)
...
while(totjobs < tjobs):
  while(pbs.get_numjobs('joseph29',queue)['R'] <= njobs):
    if(verb):
      print("Running jobs=%d"%(pbs.get_numjobs('joseph29','sep')['R']))
      print("Completed jobs=%d"%(pbs.get_numjobs('joseph29','sep')['C']))
    # Write the par file
    ...
    #cmd = bcmd + parname
    cmd = 'sleep 180'
    sfile = pbs.write_script('ptscat%d'%(k),cmd,queue=queue,logpath='log')
    # Submit the job
    pbs.submit_job(sfile,sleep=0.5,submit=submit,verb=verb)
    # Increase counters
    k += nex; totjobs += 1
    if(verb):
      print("Total jobs submitted=%d/%d"%(totjobs,tjobs))
      print("Examples run=%d"%(k))
