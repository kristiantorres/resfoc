"""
Creates residually migrated defocused images
...
import numpy as np
from cluster.resdefocslrm import resdefocjob
...
defaults = {
    "outdir": "/data/sep/joseph29/projects/resfoc/bench/dat/resdefoc",
    "prefix": "resdefoc",
    "dpath": "/data/sep/joseph29/scratch/resdefoc",
    "na": 64,
    "nro": 21,
    "oro": 1,
    "dro": 0.00125,
    ...
    "nprocs": 16,
    "nsubmit": 5,
    "logpath": "./log/resdefoc",
    "parpath": "./par/resdefoc",
    "jobprefix": "resdefoc",
    "nleft": 2,
    "delay": 10.0,
    "klean": 'y',
...
ioArgs.add_argument("-inpdir",help="Input directory containing focused images",type=str,required=True)
ioArgs.add_argument("-outdir",help="Output directory where to write the files",type=str,required=True)
ioArgs.add_argument("-iprefix",help="Prefix for input file names",type=str,required=True)
ioArgs.add_argument("-oprefix",help="Prefix for output file names",type=str,required=True)
...
# Defocusing arguments
defArgs = parser.add_argument_group('Defocusing parameters')
defArgs.add_argument("-na",help="Number of angles to compute [64]",type=int)
defArgs.add_argument("-nro",help="Number of residual migrations from which to choose [21]",type=int)
defArgs.add_argument("-oro",help="Center residual migration [1.0]",type=float)
defArgs.add_argument("-dro",help="Residual migration sampling [0.00125]",type=float)
# Other arguments
...
bcmd = '/data/biondo/joseph29/opt/anaconda3/envs/py35/bin/python ./scripts/oneresmigdefoc.py -c '
...
for ijob in range(tjobs):
  if(ijob < ajobs):
    # Create job
    actjobs.append(resdefocjob(args,jobprefix,maxnum,parpath,logpath,verb=verb))
    args.beg += 1
    # Submit job
    ...
    args.beg += 1
    lefjobs.append(resdefocjob(args,jobprefix,maxnum,parpath,logpath,verb=verb))
