...
import numpy as np
from cluster.focdefres_rndslrm import focdefres_rndjob
...
postArgs.add_argument("-offset",help="Number of samples to move from rho=1 [8]",type=int)
# Other arguments
...
bcmd = '/data/biondo/joseph29/opt/anaconda3/envs/py35/bin/python ./scripts/random_fault_focdefres.py -c '
...
for ijob in range(tjobs):
  if(ijob < ajobs):
    # Create job
    actjobs.append(focdefres_rndjob(args,jobprefix,maxnum,parpath,logpath,verb=verb))
    args.beg += 1
    # Submit job
    ...
    args.beg += 1
    lefjobs.append(focdefres_rndjob(args,jobprefix,maxnum,parpath,logpath,verb=verb))
