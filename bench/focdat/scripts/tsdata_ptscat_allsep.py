...
for iex in range(nex):
  if(verb):
    if(iex%nprint == 0):
      print("%d ... "%(beg+iex),end='')
    imgs[:,:,:,:,k], rhos[:,:,k] = createdata_ptscat(nsx,osx,nz,nx,nh,nro,oro,dro,grid=True,keepoff=True,debug=False)
  if(k == nwrite-1):
    # Create tag
    suffix = sep.create_inttag(beg+iex,end)+".H"
    # Write features
...
if(nres != 0):
  ...
  # Write features
  suffix = sep.create_inttag(beg+nex-1,end) + ".H"
