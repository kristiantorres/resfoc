"""
Converts migrated images in SEPlib
format to H5 format. Takes only the zero-subsurface
offset image and the rho=1 image
...
imgs = np.zeros([nz,nx,ntot],dtype='float32')
...
for ifile in zip(imgfiles,lblfiles):
  ...
  imgs[:,:,beg:end] = img[:,:,zidx,ro1idx,:]
  rhos[:,:,beg:end] = rho[:,:,:]
  # Update counters
  beg = end; k += 1

# Transpose
imgst = np.transpose(imgs,(2,0,1))
rhost = np.transpose(rhos,(2,0,1))
with h5py.File(out,'w') as hf:
  hf.create_dataset("data", (ntot,nz,nx), data=imgst, dtype=np.float32)
