...
romin = 0.9; romax = 0.98
rhos = vel.create_randomptb(50,300,romin,romax,ncpu=10)
# Center of anomaly
# Pad
rhosp = np.pad(rhos,((75,75),(50,50)),'constant',constant_values=1)
rhospsm = flt.gaussian_filter(rhosp,sigma=10)
plt.imshow(rhospsm,cmap='jet'); plt.show()
modvel = modvel/rhospsm
...
sep.write_file(None,raxes,rmigt,ofname='randrmigloc.H')

# Write rho map
vaxes = seppy.axes([nz,nx],[0.0,0.0],[dz,dx])
sep.write_file(None,vaxes,rhospsm,ofname='rrholoc.H')
