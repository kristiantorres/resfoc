...
ioArgs.add_argument("-cnvtrdat",help="Input convolution training dataset in H5 format",type=str,required=True)
ioArgs.add_argument("-cnvvadat",help="Input convolution validation dataset in H5 format",type=str,required=True)
ioArgs.add_argument("-migtrdat",help="Input migration training dataset in H5 format",type=str,required=True)
ioArgs.add_argument("-migvadat",help="Input migration validation dataset in H5 format",type=str,required=True)
...
# Load all data
allcnvx,allcnvy = load_alldata(args.cnvtrdat,None,105,0,600)
allmigx,allmigy = load_alldata(args.migtrdat,None,21,0,3000)

# Combine the data
allx = np.concatenate([allcnvx,allmigx],axis=0)
ally = np.concatenate([allcnvy,allmigy],axis=0)
xshape = allx.shape[1:]
yshape = ally.shape[1:]
