...
# Create dataloader
tsdgen = resmig_generator_h5(args.tsdat,args.bsize)
...
pred = model.predict_generator(tsdgen,verbose=1,steps=args.nbatches)
...
if(sepf):
  ...
  # Write the data
  sep.write_file(None,axes,pred,ofname=args.roout,dpath='/net/fantastic/scr2/joseph29/')
else:
  with h5py.File(args.roout,'w') as hf:
    nex = pred.shape[0]; nz = pred.shape[1]; nx = pred.shape[2]
    hf.create_dataset('pred', (nex,nz,nx), data=pred, dtype=np.float32)

if(qc):
  with h5py.File(args.tsdat,'r') as hf:
    keys = list(hf.keys());
    nb = int(len(keys)/2)
    nex = pred.shape[0]
    for iex in range(nex):
      f,ax = plt.subplots(1,2,figsize=(10,10),gridspec_kw={'width_ratios': [1, 1]})
      ax[0].imshow(pred[iex,:,:].T,cmap='jet')
      ax[1].imshow(hf[keys[iex+nb]],cmap='jet')
