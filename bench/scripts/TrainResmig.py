...
gpus  = sep.read_list(args.gpus,default=None)
if(gpus != None):
...
# Create dataloader
trdgen = resmig_generator_h5(args.trdat,bsize)
vadgen = resmig_generator_h5(args.vadat,bsize)
xshape,yshape = trdgen.get_xyshapes()
...
# Train the model
#history = model.fit_generator(generator=trdgen,validation_data=vadgen,
#    steps_per_epoch=self.nb, validation_steps=self.nb,
#    epochs=nepochs,shuffle=True)
history = model.fit_generator(generator=trdgen,validation_data=vadgen,epochs=nepochs,shuffle=True)
