"""
Creates many migrated seismic images
of faulted velocity models
...
import numpy as np
from cluster.migfltslrm import migjob
...
defaults = {
    "outdir": "/data/biondo/joseph29/projects/resfoc/bench/dat/migflt",
    "imgpf": "fltimg",
    "dpath": "/scratch/joseph29/migflts",
    "tjobs": 600,
    "ajobs": 200,
    "nprocs": 16,
    "nsubmit": 5,
...
bcmd = '/data/biondo/joseph29/opt/anaconda3/envs/py35/bin/python ./scripts/create_mig.py -c '
...
while ijob < tjobs:
  if(ijob < ajobs):
    ...
    for iex in range(axes.n[2]):
      actjobs.append(migjob(args,vels[k],refs[k],iex,jobprefix,parpath,logpath,verb=verb))
      # Submit job
    ...
    ctr = 0
    for iex in range(axes.n[2]):
      lefjobs.append(migjob(args,vels[k],refs[k],iex,jobprefix,parpath,logpath,verb=verb))
      ijob += 1
  k += 1
