...
sep.write_file(None,iaxes,imgo,ofname='fltimgextprcnew.H')

## Write out all shots
#datout = np.transpose(allshot,(1,2,0))
#daxes = seppy.axes([allshot.shape[1],allshot.shape[2],allshot.shape[0]],[0.0,0.0,0.0],[dtd,1.0,20])
#sep.write_file(None,daxes,datout,ofname='fltdat.H')

#TODO: need a standard header writing
#sep.to_header(None,"srcz=%d recz=%d"%(0,0),ofname='fltdat.H')
#sep.to_header(None,"bx=%d bz=%d alpha=%f"%(25,25,0.99),ofname='fltdat.H')
