...
# Read in the data
daxes,dat = sep.read_file('fltsmall.H')
...
img = prp.wem(rvelsm,allshot,wav,dtd,lap=True,verb=True,nthrds=24)
#nh,oh,dh = prp.get_off_axis()

# Write out image
#imgo = np.transpose(img,(1,2,0))
#sep.write_file('fltimgsmall.H',imgo,os=[0.0,0.0,oh],ds=[dz,dx,dh])
sep.write_file('fltimgsmall.H',img,ds=[dz,dx])
