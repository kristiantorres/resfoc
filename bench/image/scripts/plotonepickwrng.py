...
# Read in data
sep = seppy.sep()
aaxes,ang = sep.read_file('x400wrng.H')
...
# Read in semblance
saxes,semb = sep.read_file("x400sembnormwrng2.H")
semb = semb.reshape(saxes.n,order='F')
semb = np.ascontiguousarray(semb.T).astype('float32')

# Read in pick
paxes,pick = sep.read_file("x400picknormwrng2.H")
pick = pick.astype('float32')

picksm = smooth(pick,rect1=1)
...
ax.plot(picksm,z,linewidth=3,color='k')
...
ax2.plot(picksm,z,linewidth=3,color='tab:cyan')
