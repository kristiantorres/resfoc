...
class fltsegpatchchunkr:
  ...
  def __init__(self,nchnks,dat,lbl,nzp,nxp,strdz=None,strdx=None,
    ...
    patches of images and labels for fault segmentation

    Parameters:
      nchnks - length of generator (number of chunks to yield)
      dat    - input seismic images [nex,nx,nz]
    ...
    if(dat.shape != lbl.shape):
      raise Exception("Labels and data must have same shape")
    ...
    if(self.__transp):
      [self.__nex,self.__nz,self.__nx] = self.__dat.shape
    else:
      [self.__nex,self.__nx,self.__nz] = self.__dat.shape
