"""
Converts the zero-subsurface offset and rho=1
prestack residual migration image into H5 format
...
parser.add_argument("-ro1idx",help="Index rho=1 residual migration image [5]",type=int)
...
parser.add_argument("-norm",help="Normalize the migrated images [y]",type=str)
...
verb = sep.yn2zoo(args.verb); savedict = sep.yn2zoo(args.savedict);
nprint = args.nprint
prefix = args.prefix

interp = sep.yn2zoo(args.interp)
norm = sep.yn2zoo(args.interp)
dsize = args.dsize
  ...
  while nwrt < nex:
    ...
    if(k >= dsize):
      ...
      for ifit in range(nfit):
        ...
        # Save to file
        hf.create_dataset("x"+datatag, (dsize,nzf,nxf), data=imgbt, dtype=np.float32)
