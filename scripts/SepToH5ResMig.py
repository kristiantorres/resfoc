"""
Converts residually-migrated prestack images in SEPlib
format to H5 format. Takes only the zero-subsurface
offset image and all of the residual migration images
...
parser.add_argument("-norm",help="Normalize the computed image from -1 to 1 [y]",type=str)
...
nzc = iaxes.n[0]; nxc = iaxes.n[1]; nro = iaxes.n[3]
  ...
  while nwrt < nex:
    ...
    if(k >= dsize):
      ...
      for ifit in range(nfit):
        ...
        # Save to file
        hf.create_dataset("x"+datatag, (dsize,nzf,nxf,nro), data=imgbt, dtype=np.float32)
